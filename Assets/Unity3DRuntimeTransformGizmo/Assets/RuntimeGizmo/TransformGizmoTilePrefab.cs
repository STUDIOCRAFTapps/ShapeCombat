...
public class TransformGizmoTilePrefab : MonoBehaviour {
    #region Header
    [Header("References")]
    new public Camera camera;
    public Material lineMaterial;

    public TransformPivot pivot = TransformPivot.Center; // CONST

    public CenterType centerType = CenterType.All; // REMOVE
    public ScaleType scaleType = ScaleType.FromPoint; // REMOVE
    ...
    public float movementSnap = 0.0625f;
    ...
    public int maxUndoStored = 100; // REMOVE
    ...
    #region WorldEditor Addon
    public void SetTransfomType (bool isTranslate) {
        if(isTranslate) {
    ...
    public void AddTarget (Transform t) {
        Internal_AddTarget(t);
    }

    public void ClearAndAddTarget (Transform t) {
        Internal_ClearTargets();
        Internal_AddTarget(t);
    }

    public void RemoveTarget (Transform t) {
        Internal_RemoveTarget(t);
    }
    ...
    void OnDisable () {
        Internal_ClearTargets(); //Just so things gets cleaned up, such as removing any materials we placed on objects.
    ...
    #region Handles and Transforms
    public TransformSpace GetProperTransformSpace () {
        return transformType == TransformType.Rotate ? TransformSpace.Local : TransformSpace.Global;
    }
    ...
    #region Targets (Add, Remove, Clear)
    private void Internal_AddTarget (Transform target) {
        if(target != null) {
    ...
    private void Internal_RemoveTarget (Transform target) {
        if(target != null) {
            if(!targetRoots.ContainsKey(target))
                return;
    ...
    private void Internal_ClearTargets () {
        targetRoots.Clear();
        targetRootsOrdered.Clear();
        children.Clear();
    }

    private void Internal_ClearAndAddTarget (Transform target) {
        Internal_ClearTargets();
        Internal_AddTarget(target);
    }
    ...
    public float GetDistanceMultiplier () {
        ...
        if(camera.orthographic)
            return Mathf.Max(.01f, camera.orthographicSize * 2f);
