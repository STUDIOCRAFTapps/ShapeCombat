...
Shader "Universal Render Pipeline/NiloCat Extension/Screen Space Decal/Fade Unlit"
{
    Properties
    {
        [Header(Basic)]
        ...
        _FadeRange("_FadeRange (default = 0,1,0,0)", vector) = (0,1,0,0)
        ...
        SubShader
        {
            ...
            if (unity_OrthoParams.w)
            {
                ...
                //----------------------------------------------------------------------------
                float2 viewRayEndPosVS_xy = float2(unity_OrthoParams.xy * (i.screenPos.xy - 0.5) * 2);  // Ortho near/far plane xy pos 
            ...
            float3 decalSpaceUV = decalSpaceScenePos.xyz + 0.5;
            ...
            float fadeValue = invLerp(_FadeRange.x, _FadeRange.y, 1.f - decalSpaceUV.z);
            float2 uv = (((decalSpaceUV.xy - 0.5f) * max(1.f, 1.f/fadeValue)) + 0.5f) * _MainTex_ST.xy + _MainTex_ST.zw;//Texture tiling & offset
#if _FracUVEnable
                uv = frac(uv);// add frac to ignore texture wrap setting
#endif
                half4 col = tex2D(_MainTex, uv);
                col *= _Color;// tint color
                col.a = saturate((col.a * _AlphaRemap.x + _AlphaRemap.y) * fadeValue);// alpha remap MAD
